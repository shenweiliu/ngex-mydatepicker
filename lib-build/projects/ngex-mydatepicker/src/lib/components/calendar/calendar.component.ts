...
import { CalendarAttributesDirective } from "../../directives/calendar-attributes.directive";
...
import {
    DOT, UNDER_LINE, D, M, Y, DATE_ROW_COUNT, DATE_COL_COUNT, MONTH_ROW_COUNT, MONTH_COL_COUNT, YEAR_ROW_COUNT, YEAR_COL_COUNT,
    SU, MO, TU, WE, TH, FR, SA, EMPTY_STR, CLICK, STYLE, MY_DP_ANIMATION, ANIMATION_NAMES, IN, OUT, TABINDEX, TD_SELECTOR, ZERO_STR, YEAR_SEPARATOR
} from "../../constants/constants";

@Component({
    selector: "lib-calendar", //Not used as component is dynmically created to ViewContainerRef.
    templateUrl: './calendar.component.html',
    styleUrls: ['../../css/mydatepicker.style.css'],
    imports: [CommonModule, SelectionBarComponent, DayViewComponent, MonthViewComponent,
        YearViewComponent, FooterBarComponent, CalendarAttributesDirective],
...
export class CalendarComponent implements AfterViewInit, OnDestroy {
    ...
    selectorPos: IMySelectorPosition | null = null;
    ...
    initializeComponent(opts: IMyOptions, defaultMonth: IMyDefaultMonth, selectedValue: any, inputValue: string, selectorPos: IMySelectorPosition, dc: (dm: IMyDateModel, close: boolean) => void, cvc: (cvc: IMyCalendarViewChanged) => void, rds: (rds: IMyRangeDateSelection) => void, va: (va: ActiveView) => void, cbe: () => void): void {
        ...
        if (this.dayIdx !== -1) {
            ...
            for (let i = 0; i < this.weekDayOpts.length; i++) {
                //SW 1/26/2025 wrap checker for undefined.
                if (dayLabels) {
    ...
    monthViewBtnClicked(): void {
        ...
        if (this.selectMonth) {
            ...
            //SW 1/26/2025 wrap checker for undefined.
            if (this.opts && this.opts.monthLabels) {
    ...
    onMonthCellClicked(cell: IMyCalendarMonth): void {
        //SW 1/26/2025 added checker to fix Ng5 error (could also use type declarations).
        //const { currMonth, selected, disabled, name, nbr, col, row  } = cell;
        if (cell) {
            this.viewChanged = true;
    ...
    onYearCellClicked(cell: IMyCalendarYear): void {
        //SW 1/26/2025 added checker to fix Ng5 error.
        if (cell) {
            this.viewChanged = true;            
    ...
    onDayCellClicked(cell: IMyCalendarDay): void {
        //SW 1/26/2025 added checker to fix Ng5 error (could also use type declarations).
        //const { dateObj, cmo, currDay, disabledDate, markedDate, highlight } = cell;
        if (cell) {
