/** -- SW 1/26/2025 Original source code for this directive type of date picker from Kekeh angular-mydatepicker:
   https://github.com/kekeh/angular-mydatepicker.
   -- Fixed a large number of TypeScript strict type-check errors.
   -- Changed deprecated to supported code logic and scenarios. 
   -- Upgraded to structures of standalone components.
...
import {DefaultConfigService} from "./services/mydatepicker-config.service";
...
const NGX_DP_VALUE_ACCESSOR = {
    ...
    useExisting: forwardRef(() => NgExMyDatePickerDirective),
    multi: true
};
...
@Directive({
    selector: "[ngex-mydatepicker]",
    exportAs: "ngex-mydatepicker",
    providers: [UtilService, LocaleService, DefaultConfigService, NGX_DP_VALUE_ACCESSOR, NGX_DP_VALIDATORS]
})
export class NgExMyDatePickerDirective implements OnChanges, OnDestroy, ControlValueAccessor, Validator {
    ...
    private cRef: ComponentRef<CalendarComponent> | null = null;
    ...
    @HostListener(KEYUP, ["$event"]) onKeyUp(event: any) {
        ...
        if (keyCode && this.ignoreKeyPress(keyCode)) {
            return;
        }
        ...
        if (keyCode === KeyCode.esc) {
            ...
            let dateModel: IMyDateModel | null = null;
            let valid: boolean = false;
            let validateOpts: IMyValidateOptions | null = null;
            if (!dateRange) {
    ...
    public setLocaleOptions(): void {
        ...
        Object.keys(opts).forEach((k) => {
            ...
            //SW: can use this style of code.
            //let k1 = k as keyof IMyOptions;
            //if (this.opts[k1]) {
            //  this.opts[k1] = opts[k1];
            //}      
        });
    }
    ...
    public validate(c: AbstractControl): { [p: string]: any } | null {
        const value: string = this.getHostValue();
    ...
    public openCalendar(): void {
        ...
        if (this.cRef === null) {
            //SW 1/26/2025 Not use resolveComponentFactory
    ...
    public toggleCalendar(): boolean | null | undefined {
        if (this.disabled) {
            return;
        }
    ...
    public clearDate(): void {
        ...
        this.emitDateChanged({
            isRange: this.opts.dateRange || false, //SW: changed
            singleDate: {
    ...
    public setHostValue(value: string): void {
        ...
        this.hostText = value ?? '';
        //SW 1/26/2025 add checker for TypeScript error.
        ...
        const valueType: string = divHostElement && !divHostElement.enabled ? VALUE : INNER_HTML;
        value = (valueType === INNER_HTML && value === EMPTY_STR ? divHostElement?.placeholder : value) ?? '';
    ...
    private removeComponent(): void {
        //SW 1/26/2025 add checker for cRef.
        if (this.vcRef !== null && this.cRef !== null) {
    ...
    private appendSelector(elem: any): void {
        //SW 1/26/2025 added checker for BODY
    ...
    private getSelectorPosition(elem: any): IMySelectorPosition {
        ...
        //SW 1/26/2025 add checker for selectorHeight
        if (openSelectorTopOfInput && selectorHeight) {
        ...
        //SW 1/26/2025 add checker for selectorWidth
        if (alignSelectorRight && selectorWidth) {
